...
def compute_ranks(graph): # graph is url : all_links_in_url pairs
    ...
    for i in range(0, numloops): # from 0 to 10
        newranks = {}
        for page in graph: # for every key
            newrank = (1 - d) / npages
            for node in graph: # for a link in all_links_in_url
                if page in graph[node]:
                    newrank = newrank + (d * ranks[node] / len(graph[node])) 
                # 0.8*previous nr / len of outlist / len of all_links_in_url
